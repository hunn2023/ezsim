...
export default function PopularDestinations() {
  ...
  const [products, setProducts] = useState<HomeEsimProduct[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(`${API_BASE_URL}/api/catalog/products/home/esim-products`)
      .then((res) => res.json())
      .then((json) => {
        const data: HomeEsimProduct[] = json.data ?? json ?? [];
